...
- **Mandatory Agent Rule**: Created [`.agent/rules/blueteam-redteam-tdd.md`](./.agent/rules/blueteam-redteam-tdd.md) enforcing:
  - Pre-Flight Work Audit (mandatory verification of past work and CI/CD status before embarking on new tasks).
  - Test-Driven Development (TDD) cycle (Red -> Green -> Refactor) with 100% test pass requirement.
  - Blue Team (defensive secret hygiene, gitleaks, build-time injection) and Red Team (adversarial chaos tests: RED-1 to RED-5b, SSRF) gates.
- **Developer Guidelines**: Updated [`AGENTS.md`](./AGENTS.md) and [`GEMINI.md`](./GEMINI.md) to integrate Pre-Flight Work Audit and Blue/Red Team with TDD workflows.

### Security
- **Git Hygiene Guard**: Added `release/*-built*/` and `release/*-built*.zip` to `.gitignore` to ensure real token artifacts generated by `scripts/build-extension.py` remain strictly uncommitted (GUARDRAILS G1).

### Changed
- Bumped version to `0.6.4` across `package.json`, `release/chrome-extension/manifest.json`, and `packages/core/aipass-bridge/extension/manifest.json`.
