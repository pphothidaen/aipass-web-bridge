...
exports.extractCodeBlocksWithPaths = extractCodeBlocksWithPaths;
exports.isToolRefusalReply = isToolRefusalReply;
exports.buildAgentSystemPrompt = buildAgentSystemPrompt;
exports.parseAllToolCalls = parseAllToolCalls;
...
function extractCodeBlocksWithPaths(content) {
    const results = [];
    const regex = /```([^\r\n]*)\r?\n([\s\S]*?)```/g;
    let match;
...
function isToolRefusalReply(reply) {
...
function buildAgentSystemPrompt(baseDir, initialListing) {
    const toolDescriptions = tools_1.AGENT_TOOLS.map((tool) => {
...
function parseAllToolCalls(content) {
    ...
    while (i < lines.length) {
        ...
        if (editM) {
            i++;
            if (/^\s*FIND\s*$/i.test(lines[i] ?? ''))
                i++;
            const before = readUntil(['NEW', 'END']);
            if (/^\s*NEW\s*$/i.test(lines[i] ?? ''))
                i++;
            const after = readUntil(['END']);
            if (/^\s*END\s*$/i.test(lines[i] ?? ''))
                i++;
            calls.push({
...
async function runAgentLoop(userPrompt, options, initialContextText) {
    const { client, model, baseDir, maxSteps = 8, onProgress, onToolCall, } = options;
    // Grounding: get top-level listing so model immediately sees real files
    let initialListing = '';
    try {
        const listRes = await (0, tools_1.executeTool)(baseDir, 'list_dir', { path: '.' });
        ...
        if (toolCalls.length === 0) {
            ...
            if (extractedFiles.length > 0) {
                ...
                for (const fileBlock of extractedFiles) {
                    ...
                    try {
                        const toolResult = await (0, tools_1.executeTool)(baseDir, 'write_file', { path: filePath, content: fileBlock.code }, {
                        ...
                        onToolCall?.('write_file', { path: filePath }, `Tool execution failed: ${err instanceof Error ? err.message : String(err)}`);
                    }
                }
                if (anyWritten) {
...
กรุณาสร้างหรือเขียนไฟล์ตามที่ได้รับมอบหมายทันที:`,
        ...
        for (const toolCall of toolCalls) {
            ...
            const actionIcon = toolName === 'write_file' || toolName === 'replace_in_file'
                ? '✏️'
            ...
            let toolResult = '';
            try {
                toolResult = await (0, tools_1.executeTool)(baseDir, toolName, params, {
                    getDiagnostics: options.getDiagnostics,
                    onFileChange: options.onFileChange,
                });
            }
