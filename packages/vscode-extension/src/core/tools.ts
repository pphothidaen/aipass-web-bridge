...
import { ModelInfo } from '@aipass/shared';
 ...
 */
export function sanitizeWafPayload(text: string): string {
 ...
 */
export function normalizeWafPayload(text: string): string {
  if (typeof text !== 'string') return text;
  return text
    .replace(/\bif\/\* \*\/(\s*)\(/g, 'if$1(')
...
export function resolveSafePath(baseDir: string, targetPath: string): { resolvedPath: string; error?: string } {
  ...
  switch (toolName) {
    ...
    case 'open_file': {
      const filePath = String(
        params.path || params.filePath || params.file_path || params.filename || params.file || params.TargetFile || params.AbsolutePath || ''
      ).trim();
      if (!filePath) return 'Error: parameter "path" is required for read_file.';
    ...
    case 'get_file_tree': {
      ...
      function renderTree(dir: string, depth: number, prefix: string): string[] {
    ...
    case 'grep_search': {
      ...
      if (!query) return 'Error: parameter "query" is required for grep_search.';
      const subPath = String(params.path || '').trim() || '.';
      const { resolvedPath, error } = resolveSafePath(baseDir, subPath);
      if (error) return `Error: ${error}`;
      ...
      function walkAndSearch(dir: string, depth = 0) {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            try {
              ...
              if (fileStat.size > 300_000) continue;
    ...
    case 'new_file': {
      ...
      if (!filePath) return 'Error: parameter "path" is required for write_file.';
    ...
    case 'append_file': {
      ...
      if (!filePath) return 'Error: parameter "path" is required for append_to_file.';
    ...
    case 'modify_file': {
      ...
      if (!filePath) return 'Error: parameter "path" is required for replace_in_file.';
    ...
    case 'rm': {
      ...
      if (!filePath) return 'Error: parameter "path" is required for delete_file.';
      const { resolvedPath, error } = resolveSafePath(baseDir, filePath);
      if (error) return `Error: ${error}`;
    ...
    case 'mkdir': {
      ...
      if (!dirPath) return 'Error: parameter "path" is required for create_directory.';
    ...
    case 'sh': {
      ...
      if (!command) return 'Error: parameter "command" is required for run_terminal_command.';
      ...
      try {
        const result = await new Promise<{ stdout: string; stderr: string; exitCode: number }>((resolve) => {
          const runEnv: Record<string, string | undefined> = { ...process.env, CI: 'true' };
