...
import { BridgeClient, ModelInfo } from '@aipass/shared';
import { fetchAiResponse, fetchAgentResponse } from '../core/bridge';
import { getAgentBasePath } from '../core/context';
import { DEFAULT_FALLBACK_MODELS, FileChangeInfo, resolveSafePath } from '../core/tools';

export function getWorkspaceDiagnostics(targetFilePath?: string): string {
...
export class AipassViewProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'aipass.chatView';

    private _modifiedFiles = new Map<string, FileChangeInfo>();
    private _currentAbortController?: AbortController;
    private _view?: vscode.WebviewView;
    private _pendingPrompt?: string;
    private _lastActiveWorkspaceFolder?: string;

    constructor(
        private readonly _extensionUri: vscode.Uri,
        private readonly _getClient: () => BridgeClient,
    ...
    public getCachedModels(): ModelInfo[] {
        const cached = this._globalState?.get<ModelInfo[]>('aipass.cachedModels');
    ...
    public getCurrentProjectDefaultPath(): string {
        ...
        return getAgentBasePath();
    }

    public getEffectiveBasePath(explicitPath?: string): string {
        const trimmed = explicitPath?.trim();
        if (trimmed) {
            return trimmed;
        }

        const saved = this._workspaceState?.get<string>('aipass.targetPath')?.trim();
    ...
    public getBackup(pathKey: string): FileChangeInfo | undefined {
        return this._modifiedFiles.get(pathKey);
    }

    public async sendPromptFromCommand(promptText: string): Promise<void> {
        if (!this._view) {
    ...
    public resolveWebviewView(webviewView: vscode.WebviewView) {
        ...
        const chatCss = webviewView.webview.asWebviewUri(
        ...
        const initialMode = this._workspaceState?.get<string>('aipass.mode') || 'agent';
        void webviewView.webview.postMessage({
            type: 'initData',
        ...
        webviewView.webview.onDidReceiveMessage(async (data) => {
            if (data.type === 'webviewReady') {
                ...
                const savedMode = this._workspaceState?.get<string>('aipass.mode') || 'agent';
                await webviewView.webview.postMessage({
                    type: 'initData',
            ...
            if (data.type === 'openFile') {
                ...
                const { resolvedPath, error } = resolveSafePath(basePath, targetPath);
            ...
            if (data.type === 'deleteFile') {
                ...
                if (!filePath) return;
                const basePath = this.getEffectiveBasePath(data.targetPath);
                const { resolvedPath, error } = resolveSafePath(basePath, filePath);
                if (error) {
            ...
            if (data.type === 'sendMessage' || data.command === 'sendMessage') {
                ...
                try {
                    ...
                    let response = '';
                    if (mode === 'agent') {
                        response = await fetchAgentResponse(
                            this._getClient(),
                            prompt,
                            data.model,
                            targetPath,
    ...
    private _getHtmlContent(chatCss: vscode.Uri, scriptUri: vscode.Uri) {
